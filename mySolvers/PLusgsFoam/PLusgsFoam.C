...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
    ...
    while (runTime.run())
    {
        ...
        for (int intIter=0; intIter<lusgsIntIters; intIter++)
        {
            ...
            dimensionedScalar dt("dt", dimTime, pseudoTime);
            dimensionedScalar physicalTime = runTime.deltaT(); 
            Info << "Psuedo Delta T  : " << dt << endl;
            Info << "Physical Delta T: " << physicalTime << endl;
            ...
            volVectorField ddtRhoU( -dt * (fvc::ddt(rhoU))); 
            ...
            volScalarField ddtRhoE(-dt*(fvc::ddt(rhoE)));

            scalar rezRho  = fvc::domainIntegrate( mag(dRho + ddtRho) / dt ).value();
            scalar rezRhoU = fvc::domainIntegrate( mag(dRhoU+ ddtRhoU) / dt ).value();
            scalar rezRhoE = fvc::domainIntegrate( mag(dRhoE+ ddtRhoE) / dt ).value();
